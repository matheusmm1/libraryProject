...
function addBookToLibrary() {
    ...
    let numOfPages = document.querySelector('#pageNum').value;
        ...
        item.addEventListener('click', function(){
            if(index === array.length -1) {
                ...
                if(item.textContent === 'Not read') {
                    item.parentElement.setAttribute('style', 'background-color: lightcoral');
                    bookMessage.textContent = '✘ You have not read this book yet';
                    item.textContent = 'Read'
                } else if(item.textContent === 'Read') {
                    item.parentElement.setAttribute('style', 'background-color: lightskyblue');
